...
--
-- APSQL + TOTO2I (CODE BUSINESS APPLICATION / COST CENTER de l'import) :
...
BEGIN
    ...
    SELECT
        c.id,
        c."CODE BUSINESS APPLICATION",
        COALESCE(ba.cc, 'NO_COST_CENTER'),   -- COST CENTER depuis t_business_application
        c."METRIQUE DE SERVICE CODE",
        c."OFFRE",
        c."ASSET DMZR",
        c."MZR DATABASE NAME",
        c."VOLUME",
        ...
        c.uic
    FROM calc c
    -- t_business_application dédupliquée pour garantir 1 ligne par identifiant
    LEFT JOIN (
        SELECT UPPER("Unique identifier") AS uid, MIN("Cost center") AS cc
        FROM t_business_application
        GROUP BY UPPER("Unique identifier")
    ) ba ON ba.uid = UPPER(c."CODE BUSINESS APPLICATION");
